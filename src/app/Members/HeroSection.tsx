        ...
        >
          ...
          >
            ...
            <div style={{ position:'relative', zIndex:2, display:'flex', flexWrap:'wrap', gap:'18px', width:'100%', maxWidth:'980px', justifyContent:'center' }}>
              {[
                { name:'Arya Sharma',  role:'President',      photo:'https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=400&h=500&fit=crop&q=80', li:'#', ig:'#' },
                { name:'Rohan Verma',  role:'Vice President', photo:'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=400&h=500&fit=crop&q=80', li:'#', ig:'#' },
                { name:'Priya Nair',   role:'Secretary',      photo:'https://images.unsplash.com/photo-1438761681033-6461ffad8d80?w=400&h=500&fit=crop&q=80', li:'#', ig:'#' },
                { name:'Kabir Singh',  role:'Treasurer',      photo:'https://images.unsplash.com/photo-1500648767791-00dcc994a43e?w=400&h=500&fit=crop&q=80', li:'#', ig:'#' },
              ].map((m) => (
                <div
                  key={m.name}
                  style={{ position:'relative', width:'220px', height:'290px', borderRadius:'0', overflow:'hidden', flexShrink:0, boxShadow:'0 8px 24px rgba(0,0,0,0.5)', cursor:'pointer' }}
                  onMouseEnter={e => {
                    const el = e.currentTarget;
                    (el.querySelector('img') as HTMLImageElement).style.filter = 'grayscale(0)';
                    (el.querySelector('.social-overlay') as HTMLElement).style.opacity = '1';
                    (el.querySelector('.social-overlay') as HTMLElement).style.transform = 'translateX(0)';
                  }}
                  onMouseLeave={e => {
                    const el = e.currentTarget;
                    (el.querySelector('img') as HTMLImageElement).style.filter = 'grayscale(1)';
                    (el.querySelector('.social-overlay') as HTMLElement).style.opacity = '0';
                    (el.querySelector('.social-overlay') as HTMLElement).style.transform = 'translateX(12px)';
                  }}
                >
                  <img src={m.photo} alt={m.name} style={{ width:'100%', height:'100%', objectFit:'cover', display:'block', filter:'grayscale(1)', transition:'filter 0.5s ease' }} loading="lazy" />
                  ...
                  {/* Social icons */}
                  <div className="social-overlay" style={{ position:'absolute', top:'12px', right:'12px', display:'flex', flexDirection:'column', gap:'8px', opacity:0, transform:'translateX(12px)', transition:'opacity 0.3s ease, transform 0.3s ease' }}>
                    <a href={m.li} target="_blank" rel="noopener noreferrer" style={{ width:'34px', height:'34px', borderRadius:'50%', background:'rgba(255,255,255,0.15)', backdropFilter:'blur(8px)', display:'flex', alignItems:'center', justifyContent:'center', color:'#fff', border:'1px solid rgba(255,255,255,0.2)' }}>
                      <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 0 1-2.063-2.065 2.064 2.064 0 1 1 2.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>
                    </a>
                    <a href={m.ig} target="_blank" rel="noopener noreferrer" style={{ width:'34px', height:'34px', borderRadius:'50%', background:'rgba(255,255,255,0.15)', backdropFilter:'blur(8px)', display:'flex', alignItems:'center', justifyContent:'center', color:'#fff', border:'1px solid rgba(255,255,255,0.2)' }}>
                  ...
                  <div style={{ position:'absolute', bottom:'16px', left:'16px', right:'16px' }}>
                    <p style={{ margin:0, fontWeight:700, fontSize:'1rem', color:'#fff', lineHeight:1.2 }}>{m.name}</p>
                    <p style={{ margin:'4px 0 0', fontWeight:400, fontSize:'0.78rem', color:'rgba(255,255,255,0.6)' }}>{m.role}</p>
                  </div>
                </div>
              ))}
            </div>
